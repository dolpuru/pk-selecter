...
function loginmove() {
  ...
  fetch("/login", {
    ...
    body: JSON.stringify({
      pk_user_id : "202013245",
      pk_user_pw : "125734",
    }),
  }).then((response => response.json()))
  .then(json => { 
       if(json['status'] == 200){
        console.log(json['lms_data'][0]['class'])
  ...
  for(let i = 0 ; i <subject[s].length;i++){
  result = list_sequence[s].toString().indexOf(subject[s][i][0]);
  if(result >= 0){
  }else{
    list_sequence[s].push(subject[s][i][0])
  }
  }
  }
